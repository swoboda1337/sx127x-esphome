...
void SX127x::on_packet(std::vector<uint8_t> packet, float rssi, float snr) {
...
void SX127x::loop() {
  if (this->modulation_ == MOD_LORA) {
    if (this->dio0_pin_->digital_read()) {
      if ((this->read_register_(REG_IRQ_FLAGS) & PAYLOAD_CRC_ERROR) == 0) {
        ...
        if (this->frequency_ > 700000000) {
          this->on_packet(packet, (float) rssi - 157, (float) snr / 4);
        } else {
          this->on_packet(packet, (float) rssi - 164, (float) snr / 4);
        }
      }
      this->write_register_(REG_IRQ_FLAGS, 0xFF);
    }
    ...
    this->read_fifo_(packet);
    this->on_packet(packet, 0.0f, 0.0f);
  }
}
